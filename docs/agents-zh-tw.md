...
>
> **🎯 代理技能已啟用** — 57 個專業技能透過漸進式揭露擴展 Claude 在插件中的能力

## 概述

這是一個完整的生產就緒系統，結合了 **86 個專業 AI 代理**、**15 個多代理工作流編排器**、**57 個代理技能** 和 **44 個開發工具**，組織為 **66 個專注、單一用途的插件**，專為 [Claude Code](https://docs.claude.com/en/docs/claude-code/overview) 設計。
...
> - **專案版本**：v2.0.0
> - **專案最後更新**：2025-11-16
> - **文件整理時間**：2025-11-24T05:00:00+08:00
>
> **核心定位**
>
> - **功能**：86 個專業 AI 代理 + 15 個多代理編排器 + 57 個代理技能 + 44 個開發工具，組織為 66 個插件
...
- [1. 專案簡介](#1-專案簡介)
  ...
  - [1.2 核心理念](#12-核心理念)
  - [1.3 使用場景](#13-使用場景)
  - [1.4 最新更新（2025-11-16）](#14-最新更新2025-11-16)
- [2. 核心特色](#2-核心特色)
- [3. 快速開始](#3-快速開始)
- [4. 插件分類](#4-插件分類)
- [5. 代理技能系統](#5-代理技能系統)
- [6. 86 個專業代理總覽](#6-86-個專業代理總覽)
- [7. 使用指南](#7-使用指南)
- [8. 多代理工作流](#8-多代理工作流)
- [9. 最佳實踐](#9-最佳實踐)
- [10. 延伸閱讀](#10-延伸閱讀)
...
- **66 個專注插件** - 細粒度、單一用途的插件，優化最小 token 使用和可組合性
- **86 個專業代理** - 涵蓋架構、語言、基礎設施、品質、資料/AI、文檔、業務營運和 SEO 的領域專家
- **57 個代理技能** - 模組化知識套件，採用漸進式揭露的專業知識
...
1. **julia-development** - Julia 1.10+ 開發與優化
   - `julia-pro` 代理：現代 Julia 開發、效能優化、多重分派
   - 支援 Julia 1.10+ 特性、包管理、科學計算

2. **developer-essentials** - 開發者必備技能套件
   - 8 個核心技能：Git 進階、SQL 優化、錯誤處理、程式碼審查、E2E 測試、認證、除錯、Monorepo
   - 適用於所有開發場景的通用技能

3. **shell-scripting** - Shell 腳本開發
   - `bash-pro`、`posix-shell-pro` 代理
   - 3 個技能：BATS 測試模式、Bash 防禦性模式、ShellCheck 配置

#### 功能增強

1. **Temporal 工作流編排**（backend-development plugin）
   - 新增 `temporal-python-pro` 代理：Temporal 工作流編排、Saga 模式、分散式交易
   - 新增 `workflow-orchestration-patterns` 技能：設計持久化工作流、狀態管理
   - 新增 `temporal-python-testing` 技能：pytest 測試、時間跳躍、模擬策略

2. **PostgreSQL 表設計**（database-design plugin）
   - 新增 `postgresql-table-design` 技能：PostgreSQL 表設計最佳實踐

3. **支付整合安全增強**（payment-processing plugin）
   - 增強 `payment-integration` 代理的關鍵安全指南
...
**完整插件清單**: 共 26 個類別、66 個插件。查看 [完整插件目錄](https://github.com/wshobson/agents/blob/main/docs/plugins.md)。
...
**完整技能文檔**: [Agent Skills 指南](https://github.com/wshobson/agents/blob/main/docs/agent-skills.md) - 57 個專業技能
...
**完整代理參考**: [86 個代理完整清單](https://github.com/wshobson/agents/blob/main/docs/agents.md)
...
>
> **版本資訊**：Claude Code Plugins v2.0.0 - 86 個專業代理 + 66 個插件 + 57 個技能  
> **最後更新**：2025-11-24T05:00:00+08:00

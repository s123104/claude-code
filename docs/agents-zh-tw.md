...
> **⚡ 已更新至 Opus 4.5, Sonnet 4.5 & Haiku 4.5** — 三層模型策略實現最佳效能與成本平衡
>
> **🎯 代理技能已啟用** — 107 個專業技能透過漸進式揭露擴展 Claude 在插件中的能力

## 概述

這是一個完整的生產就緒系統，結合了 **99 個專業 AI 代理**、**15 個多代理工作流編排器**、**107 個代理技能** 和 **71 個開發工具**，組織為 **67 個專注、單一用途的插件**，專為 [Claude Code](https://docs.claude.com/en/docs/claude-code/overview) 設計。
...
> - **專案版本**：v3.0.0
> - **專案最後更新**：2025-12-23
> - **文件整理時間**：2025-12-24T00:55:00+08:00
>
> **核心定位**
>
> - **功能**：99 個專業 AI 代理 + 15 個多代理編排器 + 107 個代理技能 + 71 個開發工具，組織為 67 個插件
...
> - [Agent Skills 官方規範](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/overview)
...
- [1. 專案簡介](#1-專案簡介)
  ...
  - [1.2 核心理念](#12-核心理念)
  - [1.3 使用場景](#13-使用場景)
  - [1.4 最新更新（2025-12-23）](#14-最新更新2025-12-23)
- [2. 核心特色](#2-核心特色)
- [3. 快速開始](#3-快速開始)
- [4. 插件分類](#4-插件分類)
- [5. 代理技能系統](#5-代理技能系統)
- [6. 99 個專業代理總覽](#6-99-個專業代理總覽)
- [7. 使用指南](#7-使用指南)
- [8. 多代理工作流](#8-多代理工作流)
- [9. 最佳實踐](#9-最佳實踐)
- [10. 延伸閱讀](#10-延伸閱讀)
...
- **67 個專注插件** - 細粒度、單一用途的插件，優化最小 token 使用和可組合性
- **99 個專業代理** - 涵蓋架構、語言、基礎設施、品質、資料/AI、文檔、業務營運和 SEO 的領域專家
- **107 個代理技能** - 模組化知識套件，採用漸進式揭露的專業知識
- **15 個工作流編排器** - 用於複雜操作的多代理協調系統，如全端開發、安全加固、ML 管道和事件響應
- **71 個開發工具** - 優化的工具，包括專案腳手架、安全掃描、測試自動化和基礎設施設定
...
本次更新引入全新的三層（四級）模型分配策略，實現最佳效能與成本平衡：

| 層級 | 模型 | 代理數量 | 使用場景 |
|------|------|----------|----------|
| **Tier 1** | Opus 4.5 | 42 | 關鍵架構、安全審查、所有程式碼審查、生產級編碼 |
| **Tier 2** | Inherit | 42 | 複雜任務 - 用戶自選模型（AI/ML、後端、前端/行動、專業化） |
| **Tier 3** | Sonnet | 51 | 智能支援（文檔、測試、除錯、網路、API文檔、DX、遺留系統、支付） |
| **Tier 4** | Haiku | 18 | 快速操作任務（SEO、部署、簡單文檔、銷售、內容、搜尋） |

**為何關鍵代理使用 Opus 4.5？**
- SWE-bench 分數達 80.9%（業界領先）
- 複雜任務減少 65% token 消耗
- 最適合架構決策和安全審查

**Tier 2 彈性（`inherit`）**：
- 標記為 `inherit` 的代理使用您會話的預設模型
- 透過 `claude --model opus` 或 `claude --model sonnet` 設定
- 前端/行動開發者可控制成本
- AI/ML 工程師可選擇 Opus 處理複雜模型工作

**成本考量**：
- **Opus 4.5**：$5/$25 每百萬輸入/輸出 token - 關鍵工作的高級選擇
- **Sonnet 4.5**：$3/$15 每百萬 token - 效能與成本平衡
- **Haiku 4.5**：$1/$5 每百萬 token - 快速、經濟的操作

**編排模式**：
```
Opus（架構）→ Sonnet（開發）→ Haiku（部署）
```
...
- **42 個 Opus 代理** - 關鍵架構和安全審查
- **42 個 Inherit 代理** - 用戶自選模型的彈性任務
- **51 個 Sonnet 代理** - 智能支援任務
- **18 個 Haiku 代理** - 快速操作任務
- **編排模式**：`Opus（架構）→ Sonnet（開發）→ Haiku（部署）`

### 2.4 完整工具鏈

- **71 個開發工具**：專案腳手架、安全掃描、測試自動化
- **67 個斜線命令**：Git 工作流、測試生成、部署自動化
- **15 個編排器**：多代理協調處理複雜操作
...
這使所有 67 個插件可供安裝，但 **不會載入任何代理或工具** 到您的上下文。
...
| 插件                    | 描述                                         | 安裝指令                              |
| ----------------------- | -------------------------------------------- | ------------------------------------- |
| **database-design**     | 資料庫架構和設計（含 PostgreSQL 表設計技能） | `/plugin install database-design`     |
...
| **julia-development**      | Julia 1.10+ 開發與優化         | `/plugin install julia-development`      |
| **shell-scripting**        | Bash、POSIX Shell（含 3 技能） | `/plugin install shell-scripting`        |
| **developer-essentials**   | 開發者必備技能（含 8 技能）    | `/plugin install developer-essentials`   |
...
**完整插件清單**: 共 23 個類別、67 個插件。查看 [完整插件目錄](https://github.com/wshobson/agents/blob/main/docs/plugins.md)。
...
- **workflow-orchestration-patterns** - Temporal 工作流編排、Saga 模式、分散式系統
- **temporal-python-testing** - Temporal 工作流測試（pytest、時間跳躍、模擬策略）
...
- **git-advanced-workflows** - 進階 Git 工作流（rebase、cherry-pick、bisect、worktrees、reflog）
- **sql-optimization-patterns** - SQL 查詢優化、索引策略、EXPLAIN 分析
- **error-handling-patterns** - 健壯的錯誤處理（異常、Result 類型、優雅降級）
- **code-review-excellence** - 有效的程式碼審查（建設性回饋、系統化分析）
- **e2e-testing-patterns** - 可靠的 E2E 測試套件（Playwright、Cypress）
- **auth-implementation-patterns** - 認證與授權實作（JWT、OAuth2、sessions、RBAC）
- **debugging-strategies** - 系統化除錯技術、分析工具、根本原因分析
- **monorepo-management** - Monorepo 管理（Turborepo、Nx、pnpm workspaces）

#### 其他技能

- **可觀測性與監控**（4 個）：Prometheus、Grafana、分散式追蹤、SLO
- **框架遷移**（4 個）：React、Angular、資料庫、依賴升級
- **Shell 腳本**（3 個）：BATS 測試模式、Bash 防禦性模式、ShellCheck 配置
- **資料庫設計**（1 個）：PostgreSQL 表設計
- **機器學習操作**（1 個）：ML 管道工作流
- **API 腳手架**（1 個）：FastAPI 模板
- **安全掃描**（1 個）：SAST 配置

**完整技能文檔**: [Agent Skills 指南](https://github.com/wshobson/agents/blob/main/docs/agent-skills.md) - 107 個專業技能
...
| **temporal-python-pro** | Sonnet | Temporal 工作流編排、Saga 模式、分散式交易 |
...
- **elixir-pro**、**django-pro**、**fastapi-pro**、**unity-developer**、**minecraft-bukkit-pro**、**sql-pro**、**julia-pro**、**bash-pro**、**posix-shell-pro** 等
...
**完整代理參考**: [99 個代理完整清單](https://github.com/wshobson/agents/blob/main/docs/agents.md)
...
>
> **版本資訊**：Claude Code Plugins v3.0.0 - 99 個專業代理 + 67 個插件 + 107 個技能 + 71 個開發工具  
> **最後更新**：2025-12-24T00:55:00+08:00

...
> - **專案版本**：v3.1.0
> - **專案最後更新**：2025-07-24
> - **文件整理時間**：2025-11-25T02:34:00+08:00
>
> **核心定位**
...
#### 🆕 **v3.1.0 - 使用分析視圖 + v3.0.0 完全架構重寫**

**v3.1.0 新功能**：
- **📊 使用分析視圖** - 新增 `--view` 參數，支援三種時間聚合視圖
  - `--view realtime`（預設）：即時監控，包含即時更新
  - `--view daily`：每日令牌使用量，以表格格式彙總
  - `--view monthly`：每月令牌使用量，用於長期趨勢分析
...
- **📋 增強的計畫支援** - 更新限制：Pro (44k)、Max5 (88k)、Max20 (220k)、Custom (P90 基礎)
...
- **更新的計畫限制** - Pro (44k)、Max5 (88k)、Max20 (220k) 令牌
- **成本分析** - 模型特定定價與快取令牌計算
- **Rich UI** - WCAG 相容主題，自動終端背景檢測
- **使用分析視圖** - 支援即時、每日、每月三種視圖模式

#### **新 CLI 選項**
- `--view` - 使用分析視圖：realtime（預設）、daily、monthly
...
**即時視圖**（`--view realtime`，預設）：
- 即時監控，包含進度條、當前會話資料和 burn rate 分析
- 即時更新顯示
- 適合：當前會話監控、即時警告

**每日視圖**（`--view daily`）：
- 彙總每日統計，以表格格式顯示
- 顯示欄位：日期、模型、輸入/輸出/快取令牌、總令牌、成本
- 適合：每日使用模式分析、識別高峰使用日、每日預算追蹤

**每月視圖**（`--view monthly`）：
- 每月彙總資料，以表格格式顯示
- 長期趨勢分析和預算規劃
- 適合：長期趨勢分析、每月預算規劃、使用模式識別
...
| 計畫 | 令牌限制 | 成本限制 | 說明 |
|------|----------|----------|------|
| **pro** | 44,000 | $18.00 | Claude Pro 訂閱 |
...
|------|-------------|----------|--------|--------|
| **Claude Pro** | 44,000 | $18.00 | 250 | 固定限制 |
...
**使用案例**：
- **即時視圖**（`--view realtime`）：即時監控當前會話和 burn rate，包含進度條和即時更新
- **每日視圖**（`--view daily`）：分析每日消耗模式並識別高峰使用日，以表格格式顯示日期、模型、令牌統計和成本
- **每月視圖**（`--view monthly`）：長期趨勢分析和每月預算規劃，以表格格式顯示每月彙總資料
...
> **版本資訊**：Claude Code Usage Monitor v3.1.0 - 使用分析視圖 + 完全架構重寫  
> **最後更新**：2025-11-25T02:34:00+08:00  
> **專案更新**：2025-07-24T00:16:11+02:00  
> **主要變更**：v3.1.0 新增使用分析視圖（realtime/daily/monthly）、Pro 計畫限制更新為 44k、v3.0.0 完全架構重寫、套件名稱變更（`claude-usage-monitor` → `claude-monitor`）
